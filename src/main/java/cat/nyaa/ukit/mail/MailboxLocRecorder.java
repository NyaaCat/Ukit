...
public class MailboxLocRecorder {
    ...
    public MailboxLocRecorder(File recordFile) throws IOException, SQLException {
        ...
        try (var statement = connection.createStatement()) {
            statement.execute("CREATE TABLE IF NOT EXISTS " + TABLE_NAME + " (uuid TEXT PRIMARY KEY, world_x INTEGER, world_y INTEGER, world_z INTEGER, world_uuid TEXT)");
        }
    }
    ...
    public MailboxLoc getMailboxLoc(UUID playerUniqueID) throws SQLException {
        try (var statement = connection.prepareStatement("SELECT * FROM " + TABLE_NAME + " WHERE uuid = ?")) {
    ...
    public void setMailboxLoc(UUID playerUniqueID, MailboxLoc mailboxLoc) throws SQLException {
        try (var statement = connection.prepareStatement("INSERT OR REPLACE INTO " + TABLE_NAME + " (uuid, world_x, world_y, world_z, world_uuid) VALUES (?, ?, ?, ?, ?)")) {
        statement.setString(1, playerUniqueID.toString());
    ...
    public void deleteMailboxRecord(UUID playerUniqueID) throws SQLException {
        try (var statement = connection.prepareStatement("DELETE FROM " + TABLE_NAME + " WHERE uuid = ?")) {
        statement.setString(1, playerUniqueID.toString());
        statement.execute();
        }
    }
